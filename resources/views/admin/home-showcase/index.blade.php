<x-layouts.admin title="Homepage Showcase Carousel">

    <div class="flex items-center justify-between">
        <p class="text-sm text-navy-500">Photos for the small rotating carousel on the homepage (paired with the "{{ \App\Models\SiteSetting::current()->showcase_title ?: 'showcase' }}" text block).</p>
        <a href="{{ route('admin.home-showcase.create') }}" class="btn-primary shrink-0">+ Add Image</a>
    </div>
    ...
    <div class="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        @forelse ($images as $image)
            <div class="group relative overflow-hidden rounded-xl border border-navy-100 bg-white">
                ...
                <div class="absolute inset-x-0 top-0 flex justify-end gap-2 bg-gradient-to-b from-black/50 to-transparent p-2 opacity-0 transition group-hover:opacity-100">
                    <a href="{{ route('admin.home-showcase.edit', $image) }}" class="rounded-md bg-white/90 px-2 py-1 text-xs font-semibold text-navy-800">Edit</a>
                    <form method="POST" action="{{ route('admin.home-showcase.destroy', $image) }}" data-confirm="Delete this image?">
                        @csrf @method('DELETE')
        ...
        @empty
            <div class="col-span-full rounded-xl border border-dashed border-navy-200 p-12 text-center text-navy-400">
                No images yet. Add at least 2 for the carousel to rotate.
            </div>
        @endforelse
    </div>
